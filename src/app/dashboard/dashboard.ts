...
export class DashboardComponent {
  ...
  get displayName(): string {
    return this.currentUser?.username?.trim() || 'Guest';
  }

  get isMaricar(): boolean {
    const uname = this.currentUser?.username?.trim().toLowerCase() || '';
    const email = this.currentUser?.email?.trim().toLowerCase() || '';
    return uname.includes('maricar') || email.includes('maricar');
  }
  ...
  private loadCurrentUser(): void {
    try {
      ...
      if (!raw) return;
      const parsed = JSON.parse(raw) as usersModel;
      if (
        parsed &&
