...
const CURRENT_USER_STORAGE_KEY = 'currentLoggedInUser';
  ...
  onLoginSubmit() {
    ...
    if (this.loginForm.valid) {
      ...
      const matchedUser = this.newuserList.find(
        (u) => u.email.trim().toLowerCase() === String(email).trim().toLowerCase() && u.password === password
      );

      if ((isStudent || isInstructor) && matchedUser) {
        sessionStorage.setItem(CURRENT_USER_STORAGE_KEY, JSON.stringify(matchedUser));
